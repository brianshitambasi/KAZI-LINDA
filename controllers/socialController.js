...
// Accept follow request - now allows messaging
const acceptFollow = async (req, res) => {
  try {
    const { connectionId } = req.params;
    ...
    await createNotification(
      ...
      `${req.user.name} accepted your follow request - you can now message each other!`,
      { 
    ...
    res.json({ 
      message: 'Follow request accepted. You can now send messages!',
      success: true 
    });
  } catch (err) {
    console.error('Accept follow error:', err);

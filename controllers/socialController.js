...
const followUser = async (req, res) => {
  try {
    ...
    const existing = await Connection.findOne({ follower: req.user.id, following: followingId });
    if (existing) return res.status(400).json({ message: 'Already following' });
    
    await Connection.create({ follower: req.user.id, following: followingId });
...
const unfollowUser = async (req, res) => {
  try {
    await Connection.findOneAndDelete({ follower: req.user.id, following: req.params.userId });
...
const getFollowers = async (req, res) => {
  try {
    const connections = await Connection.find({ following: req.params.userId })
...
const getFollowing = async (req, res) => {
  try {
    const connections = await Connection.find({ follower: req.params.userId })
...
const checkFollowing = async (req, res) => {
  try {
    const connection = await Connection.findOne({ follower: req.user.id, following: req.params.userId });
